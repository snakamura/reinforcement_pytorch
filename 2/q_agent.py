...
class QAgent(QBaseAgent):
    def update_q(self, q, state, action, reward, next_state, next_action, eta, gamma):
        if next_state == agent.GOAL:
            ...
            q[state, action] += eta * (reward + gamma * np.nanmax(q[next_state, :]) - q[state, action])
        return q
...
if __name__ == '__main__':
    ...
    qAgent = QAgent(pi_0, eta, gamma, epsilon)
    q, state_action_history, state_values_history = qAgent.solve(q_0)
